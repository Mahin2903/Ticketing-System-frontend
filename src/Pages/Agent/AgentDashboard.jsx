...
if (typeof document !== "undefined" && !document.getElementById("swal-custom-styles")) {
  ...
  style.textContent = `
    ...
    .swal-title-custom {
      ...
      letter-spacing: -0.01em !important; padding: 0 0 4px !important;
    }
    .swal-html-custom {
    ...
    .swal-confirm-custom {
      border-radius: 12px !important; font-size: 12.5px !important;
...
const confirmDialog = ({
  title, html, confirmText = "Confirm", cancelText = "Cancel",
  icon = "question", confirmColor = "#7c3aed",
}) =>
  Swal.fire({
    ...swalTheme,
    title, html, icon,
...
const FILTER_LABELS    = {
  All: "All", PENDING: "Pending", IN_PROGRESS: "In Progress", COMPLETE: "Complete",
};
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: [
            "rgba(139,92,246,0.75)", "rgba(251,191,36,0.75)",
            "rgba(96,165,250,0.75)", "rgba(52,211,153,0.75)",
          ],
          hoverBackgroundColor: [
            "rgba(139,92,246,1)", "rgba(251,191,36,1)",
            "rgba(96,165,250,1)", "rgba(52,211,153,1)",
          ],
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            titleColor: "rgba(255,255,255,0.5)", bodyColor: "#c4b5fd",
            titleFont: { size: 10, family: "monospace" },
            bodyFont: { size: 12, weight: "bold" }, padding: 8,
            callbacks: { label: (item) => `${item.raw} ticket${item.raw !== 1 ? "s" : ""}` },
          },
        },
        scales: {
          x: { grid: { display: false }, border: { display: false }, ticks: { color: "rgba(255,255,255,0.35)", font: { size: 11 } } },
          y: { display: false, grid: { display: false }, border: { display: false }, min: 0 },
        },
      },
    });
...
const SidePanel = ({ tickets = [], users = [] }) => {
  const activeTickets = tickets.filter((t) => t.assigned_to != null && t.status !== "COMPLETE");
    ...
    <aside className="w-full mt-6 md:mt-0 lg:w-[300px] xl:w-[320px] shrink-0 flex flex-col gap-0 rounded-2xl border border-white/[0.07] shadow-xl bg-[#0a0915] overflow-hidden">
      ...
      <div className="px-5 pt-4 pb-3">
        <div className="flex items-center justify-between mb-3">
          <span className="text-[10px] font-semibold text-white/35 uppercase tracking-widest">Ticket Breakdown</span>
          <span className="text-[13px] font-bold text-violet-400">{tickets.length} total</span>
        </div>
        <div className="flex items-center gap-3 mb-2.5">
          {[{ label: "Total", color: "bg-violet-400" }, { label: "Pending", color: "bg-amber-400" }, { label: "Active", color: "bg-blue-400" }, { label: "Complete", color: "bg-emerald-400" }].map(({ label, color }) => (
        ...
        <div className="h-[140px]"><TicketStatsChart tickets={tickets} /></div>
      </div>
      <div className="px-5 pt-4 pb-3 border-t border-white/[0.06]">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] font-semibold text-white/35 uppercase tracking-widest">Agents</p>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 text-[10px] text-amber-400/70"><span className="w-1.5 h-1.5 rounded-full bg-amber-400" />{assignedAgents.length} assigned</span>
            <span className="flex items-center gap-1 text-[10px] text-emerald-400/70"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />{availableAgents.length} free</span>
          </div>
        </div>
              ...
              return (
                <div key={agent.id} className="flex items-center gap-3">
                  ...
                  {agent.busy ? (
                    ...
                    <span className="shrink-0 px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">Available</span>
                  )}
                </div>
              );
            })}
...
// ── Assign Modal (with note + SweetAlert confirm) ─────────────────────────────
const AssignModal = ({ users = [], isLoading, ticket, onSelect, onClose }) => {
  ...
  const pendingUser  = pendingUserId ? users.find((u) => u.id === pendingUserId) : null;
  const isNoteStep   = pendingUserId !== undefined;

  const stageUser = (userId) => {
    if (userId === pendingUserId) { setPendingUserId(undefined); setNote(""); return; }
  ...
  const handleConfirm = async () => {
    ...
    const isReassigning = pendingUserId && ticket.assigned_to && pendingUserId !== ticket.assigned_to;

    let title, html, confirmText, icon, confirmColor;
    if (isUnassigning) {
      title = "Remove Assignment"; icon = "warning"; confirmColor = "#f59e0b"; confirmText = "Remove";
      ...
      title = "Reassign Ticket"; icon = "question"; confirmColor = "#7c3aed"; confirmText = "Reassign";
      html  = `Switch from <strong style="color:#e2e0f0">${assignedUser?.name}</strong> to <strong style="color:#a78bfa">${pendingUser?.name}</strong>?`;
    } else {
      title = "Assign Agent"; icon = "question"; confirmColor = "#7c3aed"; confirmText = "Assign";
    ...
    const result = await confirmDialog({ title, html, confirmText, icon, confirmColor });
    if (result.isConfirmed) {
      onSelect(pendingUserId, note.trim() || null);
      showToast("success", isUnassigning ? "Assignment removed" : `Assigned to ${pendingUser?.name}`);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      >
        ...
        <div className="px-2 pb-2 max-h-56 overflow-y-auto">
          <button onClick={() => stageUser(null)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors mb-0.5 ${
              pendingUserId === null ? "bg-violet-500/15 border border-violet-500/25"
              : !ticket.assigned_to ? "bg-white/[0.06]" : "hover:bg-white/[0.04]"
            }`}>
            ...
            {(pendingUserId === null || (!ticket.assigned_to && pendingUserId === undefined)) && (
              <svg className="ml-auto text-violet-400 shrink-0" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
            )}
          </button>
          <div className="mx-1 my-1.5 border-t border-white/[0.05]" />
          {isLoading ? (
            <div className="py-8 text-center"><div className="w-5 h-5 border-2 border-violet-500/30 border-t-violet-400 rounded-full animate-spin mx-auto" /></div>
          ) : filtered.length === 0 ? (
            ...
            filtered.map((u) => {
              ...
              return (
                <button key={u.id} onClick={() => stageUser(u.id)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all ${
                    isStaged ? "bg-violet-500/20 border border-violet-500/35"
                    : isCurrentlyAssigned ? "bg-violet-500/10 border border-violet-500/20"
                    : "hover:bg-white/[0.04] border border-transparent"
                  }`}>
                  <span className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0" style={{ background: "rgba(139,92,246,0.3)", color: "#c4b5fd" }}>{initials}</span>
                  ...
                  <span className="shrink-0 px-2 py-0.5 rounded-md text-[10px] font-semibold capitalize" style={{ background: roleColor.bg, color: roleColor.text }}>{u.role ?? "user"}</span>
                  {isStaged && <svg className="text-violet-400 shrink-0" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>}
                </button>
              );
            })
          )}
        ...
        {/* Note panel — slides in once staged */}
        <AnimatePresence initial={false}>
          {isNoteStep && (
            <motion.div key="note-panel"
              ...
              <div className="px-4 pt-3 pb-4 border-t border-white/[0.07] bg-black/20">
                <div className="flex items-center gap-2 mb-2.5">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-violet-400 shrink-0">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                  </svg>
                  <span className="text-[10.5px] font-semibold text-violet-400/80 tracking-wide">
                    Assignment note <span className="text-white/25 font-normal ml-1">— optional, visible to assignee</span>
                  </span>
                </div>

                {pendingUser && (
                  <div className="flex items-center gap-2 mb-2.5 px-2.5 py-1.5 rounded-lg bg-violet-500/[0.08] border border-violet-500/15">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0" style={{ background: "rgba(139,92,246,0.35)", color: "#c4b5fd" }}>
                      {pendingUser.name.split(" ").slice(0, 2).map((w) => w[0]).join("").toUpperCase()}
                    </span>
                    <p className="text-[11.5px] text-white/60 truncate">Note will be sent to <strong className="text-violet-300 font-semibold">{pendingUser.name}</strong></p>
                  </div>
                )}
                {pendingUserId === null && (
                  <div className="flex items-center gap-2 mb-2.5 px-2.5 py-1.5 rounded-lg bg-amber-500/[0.07] border border-amber-500/15">
                    <p className="text-[11.5px] text-amber-400/70">No note required when removing assignment.</p>
                  </div>
                )}
                ...
                {pendingUserId !== null && (
                  <div className="relative">
                    ...
                    <span className={`absolute bottom-2.5 right-3 text-[10px] tabular-nums ${note.length >= NOTE_LIMIT ? "text-red-400" : "text-white/20"}`}>
                ...
                <div className="flex items-center justify-between mt-3">
                  <button onClick={() => { setPendingUserId(undefined); setNote(""); }} className="text-[11.5px] text-white/35 hover:text-white/60 transition-colors">← Back</button>
                  <button onClick={handleConfirm} className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-semibold bg-violet-600 hover:bg-violet-500 text-white transition-colors">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                    {pendingUserId === null ? "Remove assignment" : "Confirm assignment"}
                  </button>
                </div>
              </div>
            </motion.div>
...
const AgentDashboard = () => {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["agent-tickets"] });
  ...
  const deleteTicketMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: () => showToast("error", "Failed to delete ticket"),
  });
  ...
  const handleAssign = (ticketId, userId, note) =>
    assignMutation.mutate({ id: ticketId, assigned_to: userId, assignment_note: note });
  ...
  const handleSaveEdit = async (e) => {
    ...
    const currentPriority  = priority?.toUpperCase();
    const originalPriority = _original.priority?.toUpperCase();
    if (currentPriority && currentPriority !== originalPriority) changes.priority = currentPriority;
    if (status && status !== _original.status) changes.status = status;

    const cleanAssigned = assigned_to !== "" && assigned_to !== null && assigned_to !== undefined ? Number(assigned_to) : null;
    const origAssigned  = _original.assigned_to !== null && _original.assigned_to !== undefined ? Number(_original.assigned_to) : null;
    if (cleanAssigned !== origAssigned) changes.assigned_to = cleanAssigned;

    if (Object.keys(changes).length === 0) { setEditingTicket(null); return; }

    const changeLines = Object.entries(changes).map(([key, val]) => {
      const label = key === "assigned_to" ? "Agent" : key.charAt(0).toUpperCase() + key.slice(1).toLowerCase();
      const displayVal = key === "assigned_to"
        ? (val ? users.find((u) => u.id === val)?.name ?? `#${val}` : "Unassigned")
      ...
      <div className="flex gap-5">
        <div className="flex-1 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[...Array(4)].map((_, i) => <div key={i} className="h-20 rounded-2xl border border-white/[0.07] bg-white/[0.02] animate-pulse" />)}
          </div>
    ...
    <div className="flex-wrap md:flex gap-10 md:gap-5 items-start">
      <div className="flex-1 min-w-0 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-[15px] font-bold text-white/90">My Tickets</h2>
            <p className="text-[11.5px] text-white/35 mt-0.5">Manage, update, and resolve your assigned tickets</p>
          </div>
              ...
              return (
                <button key={status} onClick={() => setFilterStatus(status)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11.5px] font-medium transition-all whitespace-nowrap ${
                    filterStatus === status ? "bg-violet-500/20 text-violet-300 border border-violet-500/30" : "text-white/40 hover:text-white/70"
                  }`}>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#0a0915] shadow-xl overflow-hidden">
          ...
          {filteredTickets.length === 0 ? (
            <div className="py-20 text-center text-white/20 text-[12px]">No tickets match the selected filter.</div>
          ) : (
            <div>
              {filteredTickets.map((ticket, i) => {
                ...
                return (
                  <motion.div key={ticket.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="border-b border-white/[0.04] last:border-b-0">
                    <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-3 items-center px-5 py-3.5 transition-colors hover:bg-white/[0.02]">
                      ...
                      <div className="min-w-0 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : ticket.id)}>
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="text-[10.5px] text-white/30 truncate">
                            {departments.find((d) => d.id === ticket.department_id)?.department_title || `Dept #${ticket.department_id}`}
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] font-semibold capitalize" style={{ color: priStyle.color }}>{ticket.priority?.toLowerCase()}</span>
                      </div>

                      {/* Status Select — with SweetAlert confirm */}
                      ...
                      {/* Assign Button */}
                      <button onClick={() => setAssignModalTicket(ticket)}
                        ...
                        {assignedUser ? (
                          <>
                            <span className="w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0" style={{ background: "rgba(139,92,246,0.4)", color: "#c4b5fd" }}>{assignedUser.name[0]}</span>
                      ...
                      {/* Edit + Delete */}
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => setEditingTicket({ ...ticket, _original: { status: ticket.status, priority: ticket.priority, assigned_to: ticket.assigned_to } })}
                          title="Edit ticket" className="w-7 h-7 flex items-center justify-center rounded-lg border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.08] text-white/45 hover:text-white/80 transition-colors">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                          </svg>
                        </button>
                        <button onClick={() => handleCancelTicket(ticket)} title="Delete ticket"
                    ...
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        <motion.div key="drawer" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden">
                          <div className="px-5 pb-5 pt-3 border-t border-white/[0.05] bg-black/20 text-[12.5px] space-y-3">
                            <div>
                              <span className="text-[10.5px] font-semibold text-white/35 uppercase tracking-wider block mb-1.5">Description</span>
                              <p className="text-white/70 bg-white/[0.03] p-3 rounded-xl border border-white/[0.05] leading-relaxed">{ticket.description || "No description provided."}</p>
                            </div>
                            ...
                            {ticket.assignment_note && (
                              <div>
                                <div className="flex items-center gap-1.5 mb-1.5">
                                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-violet-400/70 shrink-0">
                                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                                  </svg>
                                  <span className="text-[10.5px] font-semibold text-violet-400/70 uppercase tracking-wider">Assignment note</span>
                                </div>
                                <p className="text-white/70 bg-violet-500/[0.07] p-3 rounded-xl border border-violet-500/20 leading-relaxed italic">"{ticket.assignment_note}"</p>
                              </div>
                            )}
                            ...
                            <div className="flex flex-wrap gap-4 text-[11.5px] text-white/40">
                              ...
                              {ticket.completed_at && <span><strong className="text-white/60">Completed:</strong> {new Date(ticket.completed_at).toLocaleString()}</span>}
                            </div>
                          </div>
                        </motion.div>
                      )}
      ...
      <AnimatePresence>
        {assignModalTicket && (
          <AssignModal
            users={users} isLoading={usersLoading} ticket={assignModalTicket}
            onSelect={(userId, note) => handleAssign(assignModalTicket.id, userId, note)}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="flex items-center justify-between border-b border-white/[0.08] pb-3">
                ...
                <button onClick={() => setEditingTicket(null)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/[0.08] text-white/40 hover:text-white transition-colors">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
                </button>
              </div>
              ...
              <form onSubmit={handleSaveEdit} className="space-y-3">
                <div>
                  ...
                  <input className={`${inputCls} opacity-60 cursor-not-allowed`} value={editingTicket.subject} readOnly />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {PRIORITY_OPTIONS.map((p) => {
                        const isActive = editingTicket.priority?.toUpperCase() === p;
                        const s = PRIORITY_STYLE[p];
                        return (
                          <button key={p} type="button" onClick={() => setEditingTicket((prev) => ({ ...prev, priority: p }))}
                            className="py-1.5 rounded-lg text-[11px] font-semibold transition-all border text-center"
                            style={{ color: isActive ? s.color : "rgba(255,255,255,0.3)", background: isActive ? s.bg : "rgba(255,255,255,0.03)", borderColor: isActive ? s.color + "55" : "rgba(255,255,255,0.07)" }}>
                  ...
                  <div>
                    ...
                    <select className={inputCls} value={editingTicket.status}
                ...
                <div>
                  ...
                  <select className={inputCls} value={editingTicket.assigned_to ?? ""}
                    onChange={(e) => setEditingTicket((prev) => ({ ...prev, assigned_to: e.target.value ? parseInt(e.target.value, 10) : null }))}
                    style={{ colorScheme: "dark" }}>
                ...
                <div>
                  ...
                  <textarea rows="3" readOnly className={`${inputCls} opacity-60 cursor-not-allowed resize-none`} value={editingTicket.description ?? ""} />
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  <button type="button" onClick={() => setEditingTicket(null)}
                    className="px-4 py-2 rounded-xl text-xs font-medium border border-white/[0.09] text-white/60 hover:bg-white/[0.05] transition-colors">Cancel</button>
