...
const AgentDashboard = () => {
  ...
  const handleSaveEdit = (e) => {
    ...
    // Check Priority (Normalized to UPPERCASE)
    const currentPriority = priority?.toUpperCase();
    const originalPriority = _original.priority?.toUpperCase();
    if (currentPriority && currentPriority !== originalPriority) {
      changes.priority = currentPriority;
    }

    // Check Status
    if (status && status !== _original.status) {
      changes.status = status;
    }

    // Check Assigned User (Normalized to Number/null)
    const cleanAssigned =
      assigned_to !== "" && assigned_to !== null && assigned_to !== undefined
        ? Number(assigned_to)
        : null;
    const origAssigned =
      _original.assigned_to !== null && _original.assigned_to !== undefined
        ? Number(_original.assigned_to)
        : null;

    if (cleanAssigned !== origAssigned) {
      changes.assigned_to = cleanAssigned;
    }

    // If nothing changed, close modal without making an API call
                ...
                <div className="grid grid-cols-2 gap-3">
                      ...
                      {PRIORITY_OPTIONS.map((p) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setEditingTicket((prev) => ({ ...prev, priority: p }))
                            }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditingTicket((prev) => ({ ...prev, status: e.target.value }))
                      }
